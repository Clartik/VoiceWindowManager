...
- `action` — one of `open`, `close`, `restore`, `move`, `dock`, `assign_desktop`, `switch_desktop`, `exit`, `unknown`.
  ...
  - `dock` covers snapping into a fixed screen position (halves/quarters), maximizing (`position: "full"`), and minimizing (`position: "empty"`), and also absorbs directional phrasing like "move it left" — there is intentionally no separate freeform-nudge action. Minimize was originally its own top-level action but was folded into `dock` (`position: "empty"`) since it's conceptually another window "screen state," alongside maximize/snap positions.
  - `restore` is a **separate action from `dock`+`full`** — restoring returns a window to its normal size from minimized/maximized, which is not the same as maximizing. This mirrors `pywinctl`, which exposes `.restore()` as a distinct method from `.maximize()`; the two were briefly conflated in an earlier prompt revision and then deliberately split apart.
  - `dock`'s (and previously `minimize`'s) `target` accepts the literal string `"all"` (for "hide windows"/"minimize everything"/"maximize everything") in addition to a specific app/window name — this is a special-cased value, not a separate schema field. A bare command with no app named and no plural/"everything" language (e.g. "minimize", "maximize") must default to `null` (focused window), not `"all"` — this was a real bug where the model over-generalized `"all"` for unqualified commands.
...
- `position` — a closed `Literal` enum (`left_half`, `right_half`, `top_half`, `bottom_half`, `top_left`, `top_right`, `bottom_left`, `bottom_right`, `full`, `empty`), used only by `dock`. Kept as an enum (unlike `target`/`destination`) because the window-management code can only compute a fixed set of positions. `full` = maximize, `empty` = minimize.
